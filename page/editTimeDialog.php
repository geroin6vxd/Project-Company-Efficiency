...
$event = $project->get_timesheet_event($id);
$currProject = $project->get_project($event->PROJECT, false, true);
?>
<table style="width: 100%">
	<tr>
		<td class="finetext">Project:*</td>
		<td id="projectTD"><?=$project->get_project_chosen($currProject->ID, "project", false, true)?></td>
	</tr>
	<tr><td>&nbsp;</td><td></td></tr>
	<tr>
		<td class="finetext">Project Code:*</td>
		<td id="projectCodeTD"><input type="text" placeholder="Project code will filled automatically" class = "finestr textcenter" readonly value=" <?=$currProject->PROJ_YEAR.' '.$currProject->PROJ_TYPE.' '.$currProject->PROJ_NUM?>"></td>
	</tr>
	<tr><td>&nbsp;</td><td></td></tr>
	<tr>
		<td class="finetext">Office:*</td>
		<td id="officeTD"><?=$project->get_offices_chosen($event->OFFICE, "off")?></td>
	</tr>
	<tr><td>&nbsp;</td><td></td></tr>
	<tr class = "realRow">
		<td class="finetext">Task:*</td>
		<td id="taskTD"><?=$project->get_task_chosen($event->TASK, "task")?></td>
	</tr>
    <tr class = "realRow phaseRow"><td>&nbsp;</td><td></td></tr>
	<tr class = "realRow phaseRow">
		<td class="finetext">Phase:*</td>
		<td id="phaseTD"><?=$project->get_phases_chosen($event->PHASE, "phas")?></td>
	</tr>
    <tr class = "realRow"><td>&nbsp;</td><td></td></tr>
	<tr>
		<td class="finetext">Hours:*</td>
		<td id="hoursTD"><input type="text" id = "hours" placeholder="Enter the number of hours" class = "finestr textcenter" value="<?=$event->HOURS?>"</td>
	</tr>
</table>
<table style="width: 100%">
	<tr class = "textcenter">
		<td style="width: 33%"><button class="finebutton" id="save">Save</button></td>
		<td style="width: 33%"><button class="finebutton" id="del">Delete</button></td>
		<td style="width: 33%"><button class="finebutton" id="cancel">Cancel</button></td>
	</tr>
</table>
<SCRIPT>
	...
	$(".finebutton#del").unbind().click(function(){
			$.ajax({
			method: "POST",
			url: "page/executor.php",
			datatype: JSON,
			data: {
				operation: "DELETE_TIMESHEET_EVENT",
				id: <?=$id?>
			}
		})
			.done(function () {
	...
	$(".finebutton#save").unbind().click(function(){
        ...
        if(nums && selections && addSelections && addSelectionsPhas){
			$.ajax({
				...
				datatype: JSON,
				data: {
					operation: "EDIT_TIMESHEET_EVENT",
					id: <?=$id?>,
					project:$("#project").val(),
					office:$("#off").val(),
					phase:$("#phas").val(),
