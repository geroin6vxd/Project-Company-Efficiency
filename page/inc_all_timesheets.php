...
$tmp = $user->get_rated_users();
foreach ($tmp as $uitem) {
	$id = $uitem->ID;
	$currUser = $user->get_users($id);
	?>
	...
	<div>
		<table id="tsheet" border=0 cellpadding=0 cellspacing=0 width=1102 class=xl659480
			...
			<?
			$res = $project->get_timesheet($id, $first_of_week, 604799, FALSE);
			$sharr = array();
			$count = 1;
			foreach ($res as $item) {
				$sharr[$item->PROJECT][$item->OFFICE][$item->TASK][$item->PHASE][] = array($item->P_DATE, $item->HOURS, $item->ID);
			}
					...
					foreach ($task_array as $task => $phase_array) {
						...
						foreach ($phase_array as $phase => $date_array) {
							...
							for ($i = 1; $i <= 7; $i++) {
								$value = '';
								$tdid = '';
								$class = ($i > 5) ? 'hourhell xl939480 empty' : 'hourhell empty';
								foreach ($date_array as $item) {
									if (($item[0] >= $curr_date_begin) && ($item[0] < $curr_date_end)) {
										$value = $item[1];
										$tdid = 'id=' . $item[2];
										$class = ($i > 5) ? 'hourhell xl939480 filled' : 'hourhell filled';
									}
								}
...
<SCRIPT>
	...
	$(".sumweek").each(function () {
		var sum = 0;
		$(this).siblings(".hourhell").each(function () {
	...
	for (var i = 1; i <= 7; i++) {
		$(".allday" + i).each(function () {
			var sum = 0;
			$(this).parent().parent().find(".day" + i).each(function () {
				sum += Number($(this).text()) || 0;
			});
			$(this).text(sum);
		});
